...
interface EstatePremiumCardProps {
  className?: string;
  onAcquistaClick?: () => void;
}

const EstatePremiumCard: React.FC<EstatePremiumCardProps> = ({ className, onAcquistaClick }) => {
    ...
    <div className={`relative w-full h-full overflow-hidden rounded-xl bg-white ${className || ''}`} lang="it">
      ...
      <div className="relative z-10 p-5">
        ...
        <div className="text-center mb-4">
          <h1 className="text-4xl sm:text-5xl font-bold text-sky-500 font-script tracking-wide transform -rotate-2">{t('packages.premium')}</h1>
          <p className="text-base sm:text-lg font-bold uppercase tracking-widest mt-1">{t('estatePremium.subtitle', 'FESTA E MARE NO STOP!')}</p>
        </div>
        ...
        <div className="text-center mb-3">
          <p className="font-bold text-base">{t('estatePremium.validOffer', 'Offerta valida per le prime 50 persone!')}</p>
        </div>
        
        {/* Lista caratteristiche */}
        <div className="space-y-2 mb-4">
          <FeatureItem text={t('estatePremium.feature1', '5 giorni di Luglio di pura emozione e follia')} />
          <FeatureItem text={t('estatePremium.feature2', 'Hotel sul mare con festa in piscina da paura')} />
          <FeatureItem text={t('estatePremium.feature3', 'Feste tutte le sere; incluso boat party con alcohol illimitato')} />
          <FeatureItem text={t('estatePremium.feature4', 'Accesso VIP a tutti i nostri eventi')} />
          <FeatureItem text={t('estatePremium.feature5', 'Sconti esclusivi su escursioni, beach club, ristoranti e attività')} />
        </div>
        
        {/* Opzione bus - in grassetto e senza spunta */}
        <div className="mb-4 text-center">
          <p className="font-bold text-sm">{t('estatePremium.busOption', 'OPZIONE BUS DALLA TUA CITTA E TRAGHETTO ANDATA E RITORNO AGGIUNGENDO 200€')}</p>
        </div>
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="bg-sky-500 text-white rounded-full p-3 sm:p-4 shadow-lg transform rotate-3">
            <div className="text-center">
              <div className="text-xs font-semibold">{t('estatePremium.justFor', 'A SOLI')}</div>
              <div className="text-xl sm:text-3xl font-bold">279€</div>
            </div>
          </div>
        </div>
      </div>
...
export default EstatePremiumCard; 